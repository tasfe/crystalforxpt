package com.combanc.monitor.vo.urlMonitor.https;
...
import com.combanc.monitor.vo.urlMonitor.https.httpclientssl.BaseSecureProtocolSocketFactory;
...
public class HTTPSMonitor extends AbstractMonitor {
	...
	public MonitorResult doMonitor(MonitorUrlResponse hur) {
		...
		Protocol myhttps = new Protocol("https",
				new BaseSecureProtocolSocketFactory(), 443);
		Protocol.registerProtocol("https", myhttps);

		HttpClient client = new HttpClient();

		client.getParams().setSoTimeout(25000);
	...
	public static void main(String[] args) throws Exception {
		HTTPSMonitor m = new HTTPSMonitor();
		MonitorUrlResponse p = new MonitorUrlResponse();
		p.setPort("443");
		p.setAddress("/app/V5/CN/STY1/login.jsp");
		p.setContain("建设银行");
		p.setNocontain("行");
		p.setServerIp("ibsbjstar.ccb.com.cn");
		p.setReplyTime(25);
		long now = System.currentTimeMillis();
/*		MonitorResult r = m.doMonitor(p);
		System.out.println("time:" + (System.currentTimeMillis() - now));
		System.out.println(r.getResultDesc());
		System.out.println("wonted:" );*/
	}
}