package com.combanc.itsm.service.notice;
...
public class PersonalNoticeService {
	...
	//用于定时器发送个人通知;
	public void sendNotice2(Integer userId,String to,String title,String content) {
		PersonalNotice bean = new PersonalNotice();
		bean.setUserId(userId);
		bean.setTitle(title);
		bean.setContent(content);
	...
	{
		return this.systemNoticeDAO.listNewPersonalNoticeByUserId(userId);
	}
	...
	public PersonalNotice getTopPersonalNotice(){
		PersonalNotice pn=null;	
		String hql="from PersonalNotice as model where model.read=0 order by date ASC";
		return pn;
	}
	
	public void updatePersonNotice(PersonalNotice personalNotice){
		systemNoticeDAO.updatePersonNotice(personalNotice);
	}
    public PersonalNotice findByUserId(int userId)
    {
    	return systemNoticeDAO.getPersonalNoticeByUserId(userId);
    }
}
