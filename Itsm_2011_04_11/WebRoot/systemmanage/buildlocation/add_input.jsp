...
function confir(){
	var value1=document.getElementById('buildlocation.name').value;
	if(value1==''){ alert('请输入位置名称！'); return false;}
	var value2=document.getElementById('buildlocationname').value;
	if(value2==''){ alert('请选择父级位置！'); return false;}
...
-->
	...
	<body leftmargin="0" marginwidth="0" onMouseDown="notshow();" >
		<s:form id="form1" action="%{actionURI}.action" method="post" theme="simple">
			<s:hidden id="buildlocation.id" name="buildlocation.id"></s:hidden>
			<table width="80%" height="75" border="0" cellspacing="1"
				...
				<tr style="height: 25">
					...
					<td width="88%" bgcolor="#FFFFFF">
						<s:textfield id="buildlocation.name" name="buildlocation.name"
...
&nbsp;<input type="text" name="buildlocationname" id="buildlocationname" onClick="document.getElementById('buildlocation').style.visibility='visible'" readonly value="<s:property value="buildlocation.name" />" style="width:80%;">
