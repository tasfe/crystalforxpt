		...
		<table width="100%" height="100%" border="0" cellpadding="2"
			...
			<tr>
				<td height="100%" colspan="2" valign="top" bgcolor="#FFFFFF"
					style="padding: 0px">
					<div
						style="position: absolute; overflow-x: hidden; overflow-y: scroll; height: 152px; width: 100%; padding-right: 3px; padding-bottom: 3px;">
						<script type="text/javascript">
													d = new dTree('d');
												
													<s:iterator value="locations" var="location">
														d.add('${location.id}'-1,'${location.pid}'-1,'${location.name}','list.action?pid=${location.id}','','listFrame');
													</s:iterator>
													document.write(d);
													d.openAll();
												</script>
