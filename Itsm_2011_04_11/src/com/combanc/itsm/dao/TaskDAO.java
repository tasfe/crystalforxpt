...
public class TaskDAO extends BaseHibernateDAO<TaskImpl, Integer> {
	public TaskDAO() {
		super();
	}

	public List<TaskImpl> getTasksByUserId(String userId, PageBean pb) {
		Users user = (Users) getHibernateTemplate().load(Users.class,
				new Long(userId));
		Iterator rolesIt = user.getRoleteams().iterator();
		StringBuffer groupIds = new StringBuffer();
		int i = 0;
		while (rolesIt.hasNext()) {
			if (i++ > 0)
				groupIds.append(",");
			groupIds.append("'" + ((Users) rolesIt.next()).getId().toString()
					+ "'");
		}
		...
		hqlSb
				.append(" or ( task.assignee is null and pt.type = 'candidate' and ((pt.userId=?)");

		if (user.getRoleteams().size() > 0) {
	...
	public List<Integer> getGroupByTask(Long taskId) {
