...
import com.combanc.common.jbpm.jpdl.Node;
...
public class JbpmService extends BaseService<ProDefinition, Integer> {
	private static final Log logger = LogFactory.getLog(JbpmService.class);
	...
	public void doUnDeployProDefinition(Integer defId) {
	...
	public ProDefinition saveOrUpdateDeploy(ProDefinition proDefinition) {
		if (proDefinition.getDeployId() == null) {
			...
			ProDefinition proDef = (ProDefinition) this.proDefinitionService
					.get(proDefinition.getDefId().intValue());
	...
	public ProcessDefinition getProcessDefinitionByKey(String processKey) {
		List list = this.repositoryService.createProcessDefinitionQuery()
				.processDefinitionKey(processKey).orderDesc(
						"versionProperty.longValue").list();
	...
	public String getDefinitionXmlByDefId(Integer defId) {
	...
	public List<Node> getTaskNodesByDefId(Integer defId) {
		ProDefinition proDefinition = (ProDefinition) this.proDefinitionService
				.get(defId);
	...
	public List<Node> getFormNodes(Integer defId) {
		ProDefinition proDefinition = (ProDefinition) this.proDefinitionService
				.get(defId);
	...
	public String startProcess(String deployId, Map variables) {
		...
		ProcessInstance pi = this.executionService.startProcessInstanceById(pd
				.getId(), variables);
	...
	public void assignTask(ProcessInstance pi, ProcessDefinition pd,
		...
		if (StringUtils.isNotEmpty(taskName)) {
			taskList = this.taskService.createTaskQuery().processInstanceId(
				...
				if (assign != null) {
					if ("__start".equals(assign.getUserId())) {
						...
						if ("__super".equals(assign.getUserId())) {
							...
							/*
							 * if (flowStartUser != null) { List superUserIds =
							 * this
							 * .userSubService.upUser(flowStartUser.getId());
							 * upIds = new StringBuffer(); for (localObject =
							 * superUserIds.iterator(); ((Iterator)
							 * localObject).hasNext();) { userId = (Long)
							 * ((Iterator) localObject).next();
							 * upIds.append(userId).append(","); }
							 * 
							 * if (superUserIds.size() > 0)
							 * upIds.deleteCharAt(upIds.length() - 1); else {
							 * upIds.append(flowStartUser.getId()); }
							 * this.taskService
							...
							if ((userIds != null) && (userIds.length > 1)) {
								/*
								 * userId = (localObject = userIds).length; for
								 * (upIds = 0; upIds < userId; ++upIds) { String
								 * uId = localObject[upIds];
								 * this.taskService.addTaskParticipatingUser(
								 * task.getId(), uId, "candidate"); }
								 */
							} else {
								this.taskService.assignTask(task.getId(),
										assign.getUserId());
	...
	public List<Task> getTasksByPiId(String piId) {
		List taskList = this.taskService.createTaskQuery().processInstanceId(
				piId).list();
		return taskList;
	}
