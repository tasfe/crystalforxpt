 ...
 * @version 1.0
 */
public class ComputerAlertParser extends BaseParser{

	private List<MonitorAlert> computerAlertList = null;

	public List<MonitorAlert> getComputerAlertList() {
		return computerAlertList;
	}

	public void setComputerAlertList(List<MonitorAlert> computerAlertList) {
		this.computerAlertList = computerAlertList;
	}
	
	public void checkAlert() {
		this.init();
		// 若计算机类报警异常列表为空，则直接返回
		if( null == computerAlertList || computerAlertList.isEmpty() 
		...
		for( int i=0; i<computerAlertList.size(); i++ ){
			MonitorAlert a = computerAlertList.get(i);
			...
			if( null == dv ){
				computerAlertList.remove(i);
				i--;
				continue;
			}
			// 如果没有给该设备绑定报警策略，控制台报警后直接继续
			// 计算机类报警，不进行声音报警
			if( null == dv.getMonitorAlertPolicy() ){
				...
				String msg = (a.getMonitorAlertSmalltype().getName().equals("新计算机") ? "" : "，档案值："
							+ a.getOld())
							+ "，计算机IP：" + a.getIp() 
							+ "，计算机MAC：" + a.getMac()
							+ "，计算机上连设备：" + a.getUplink()
